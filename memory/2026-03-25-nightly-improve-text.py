...
TEXT_LOG_SUFFIX = "nightly-improve-text.log"
...
TEXT_LOG_PATH = os.path.join(MEMORY_DIR, f"{LOG_PREFIX}-nightly-improve-text.log")
...
def log_tick(ts, improvements, log_path):
    last = _read_last_json_line(log_path)
    last_improvements = last.get("improvements", []) if last else []
    delta = [imp for imp in improvements if imp not in last_improvements]
    line = {
        "timestamp": ts,
    ...
    # Human-readable log
    with open(TEXT_LOG_PATH, "a", encoding="utf-8") as f_text:
        f_text.write(f"[{ts}] Improvements:\n")
        for imp in improvements:
            f_text.write(f"- {imp}\n")
        if delta:
            f_text.write("Delta since last tick:\n")
            for d in delta:
                f_text.write(f"  - {d}\n")
        f_text.write("\n")
...
def main():
    ...
    print("Nightly improvement v2 (text log with delta) finished at", dt.datetime.now())
